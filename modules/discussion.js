...
app.get('/article/:id', async (req, res) => {
  try {
    ...
    if(article.is_blog) throw new ErrorMessage('博客文章禁止在讨论区访问。');
    await article.loadRelationships();
...
app.get('/article/:id/edit', async (req, res) => {
  try {
    ...
    if(req.query.is_blog){
      article.is_blog=true;
    }
    await article.save();
...
app.post('/article/:id/edit', async (req, res) => {
  try {
    ...
    if(req.query.is_blog && req.body.image_url != "default"){
    ...
    await article.save();
    if(req.query.is_blog){
      res.redirect(syzoj.utils.makeUrl(['user',res.locals.user.id,'blog',article.id]));
    }
    else{
...
app.post('/article/:id/delete', async (req, res) => {
  try {
    ...
    if(article.is_blog){
      res.redirect(syzoj.utils.makeUrl(['user',res.locals.user.id,'blog']));
    }
    else{
      res.redirect(syzoj.utils.makeUrl(['discussion', 'global']));
    }
