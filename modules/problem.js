...
app.get('/problems', async (req, res) => {
  try {
    ...
    let page_id=req.query.page;
    if(!req.query.page)page_id=999;


    let paginate = syzoj.utils.paginate(await Problem.countForPagination(query), page_id, syzoj.config.page.problem);
...
app.get('/problem/:id/export', async (req, res) => {
  try {
    let id = parseInt(req.params.id);
    let sercet = req.query.sercet;
    let problem = await Problem.findById(id);
    if (!problem) throw new ErrorMessage('无此题目。');
    if (!problem.is_public){
      if(sercet != syzoj.config.judge_token){
        throw new ErrorMessage('sercet错误。');
      }
    }
    let obj = {
      title: problem.title,
...
app.get('/problem/:id/load', async (req,res) => {
  try {
    let id = parseInt(req.params.id) || 0;
    let problem = await Problem.findById(id);
...
app.get('/problem/:id/load_file', async (req,res) => {
  try {
    let id = parseInt(req.params.id) || 0;
    let problem = await Problem.findById(id);
...
app.post('/problem/:id/load', async (req,res) => {
    try {
    let id = parseInt(req.params.id) || 0;
    let problem = await Problem.findById(id);
    if (!problem) {
      ...
      //problem.user_id = res.locals.user.id;
      //problem.publicizer_id = res.locals.user.id;
      problem.user_id=problem.publicizer_id=1;
    } else {
      //if (!await problem.isAllowedUseBy(res.locals.user)) throw new ErrorMessage('您没有权限进行此操作。');
      //if (!await problem.isAllowedEditBy(res.locals.user)) throw new ErrorMessage('您没有权限进行此操作。');
    }
    ...
    let url = require('url');
    let json_string=req.body.json;
    //let json_string="{\"success\":true,\"obj\":{\"title\":\"A+B\",\"description\":\"A+B\",\"input_format\":\"\",\"output_format\":\"\",\"example\":\"%23%23%23%23 \\u6837\\u4F8B\\u8F93\\u5165\\r\\n```plain\\r\\n1 2\\r\\n```\\r\\n%23%23%23%23 \\u6837\\u4F8B\\u8F93\\u51FA\\r\\n```plain\\r\\n3\\r\\n```\",\"limit_and_hint\":\"\",\"time_limit\":1000,\"memory_limit\":256,\"have_additional_file\":false,\"file_io\":false,\"file_io_input_name\":null,\"file_io_output_name\":null,\"type\":\"traditional\",\"tags\":[]}}";
    //throw new ErrorMessage(json_string,null,json_string);
...
app.post('/problem/:id/import', async (req, res) => {
  try {
    ...
    let json = await request({
      uri: req.body.url + (req.body.url.endsWith('/') ? 'export' : '/export'+'?sercet='+req.params.sercet),
      timeout: 1500,
      json: true
    });
...
app.post('/problem/:id/load_file', app.multer.fields([{ name: 'data', maxCount: 1 }]),async (req,res) => {
  try {
    let id = parseInt(req.params.id) || 0;
    let problem = await Problem.findById(id);
    if (!problem){
...
app.post('/problem/:id/submit', app.multer.fields([{ name: 'answer', maxCount: 1 }]), async (req, res) => {
  try {
    ...
    if(req.body.sanitize=="on"){
      judge_state.use_sanitize=true;
    }
    else{
      judge_state.use_sanitize=false;
    }
    try {
...
app.get('/problem/:id/testdata/download_export', async (req, res) => {
  try {
    let id = parseInt(req.params.id);
    let problem = await Problem.findById(id);
    ...
    let path = require('path');
    let filename = problem.getTestdataExportPath();
    if (!await syzoj.utils.isFile(filename)) throw new ErrorMessage('文件不存在。');
...
app.post('/problem/image_upload_base64', async (req, res) => {
  try {
      if(!res.locals.user.is_admin)throw new ErrorMessage('您没有权限进行此操作。'); //只有管理员有上传图片权限
      let params = req.body;
      if(!params.base64str)throw new ErrorMessage('参数不正确。');
      let imgData = params.base64str;

      let fileArr = imgData.match(/data\:image\/(\w+)\;/);
      if(!fileArr || fileArr.length < 2)throw new ErrorMessage('不支持的格式。');
      let filename = `images/${moment().format("YYMMDD")}_${randomstring.generate(10)}.${fileArr[1]}`;

      //过滤data:URL
      let base64Data = imgData.replace(/^data:image\/\w+;base64,/, "");
      let dataBuffer = new Buffer(base64Data, 'base64');
      fs.writeFile(`static/${filename}`, dataBuffer, function(err) {
          if(err){
              res.send({code: -1, msg: "保存图片失败"});
          }else{
              res.send({code: 0, msg: "success", filename: filename});
          }
      });

  } catch (e) {
      res.send({code: -1, msg: e.toString()});
  }
});

app.post('/problem/image_upload', async (req, res) => {
  try {
      if(!res.locals.user.is_admin)throw new ErrorMessage('您没有权限进行此操作。'); //只有管理员有上传图片权限
      ...
      form.parse(req, function (err, fields, files){
          let filename = files.tmpFile.name
          let nameArray = filename.split('.');
          let type = nameArray[nameArray.length - 1];
          let filepath = `images/${moment().format("YYMMDD")}_${syzoj.utils.getRndStr(10)}.${type}`;
          let newPath = `static/${filepath}`;
          fs.renameSync(files.tmpFile.path, newPath);  //重命名
          res.send({code: 0, msg: "success", filename: filepath});
      })
  } catch (e) {
      res.send({code: -1, msg: e.toString()});
  }
...
app.post('/problem/upload',(req,res)=>{
  try{
    var form = new formidable.IncomingForm(),picture;
    form.parse(req, function (err, fields, files) {
      if (err) {
        console.log("error:", err);
      }
      let dir=files.file.path;
      let data=fs.readFileSync(dir);
      let name=randomstring.generate(10);
      fs.writeFileSync(`static/images/`+name,data);
      res.send({success:true,message:"",url:`images/${name}`});
    });
  }
  catch(e){
