...
app.get('/contest/:id/access',async(req,res,next)=>{
  let contest_id = parseInt(req.params.id);
  let contest = await Contest.findById(contest_id);
  if(contest.password && contest.password != req.query.password){
    res.redirect("/contest/"+contest_id+"?fail=true");
    return;
  }
  if(contest.password){
    req.session['contest-'+contest_id]=contest.password;
  }
  res.redirect(req.query.back);
});

app.get('/contest/:id*',async(req,res,next)=>{
  try{
    if(req.query.isBot){
      next();
      return;
    ...
    if(contest.password&&req.session['contest-'+contest_id]==contest.password){
      next();
      return;
    }
    if(contest.password && contest.password!=pwd){
      res.render('contest_confirm',{
...
app.get('/contest/:id/after_ranklist' , async(req, res)=> {
  try {
    let contest_id = parseInt(req.params.id);
    let contest = await Contest.findById(contest_id);
    ...
    let players_id = [];
    for (let i = 1; i <= contest.after_ranklist.ranklist.player_num; i++) players_id.push(contest.after_ranklist.ranklist[i]);
    ...
    let ranklist = await players_id.mapAsync(async player_id => {
      ...
      for (let i in player.score_details) {
        ...
        if (contest.type === 'noi' || contest.type === 'ioi') {
          let multiplier = (contest.after_ranklist.ranking_params || {})[i] || 1.0;
...
app.get('/contest/:id/download' , async(req, res) => {
  try {
    let contest_id = parseInt(req.params.id);
    let contest = await Contest.findById(contest_id);
    res.download(contest.getXmlRanklistPath());
  } catch (e) {
    syzoj.log(e);
    res.render('error', {
      err: e
...
app.post('/contest/:id/edit', async (req, res) => {
  try {
    ...
    let contest_id = parseInt(req.params.id);
    let contest = await Contest.findById(contest_id);
    let ranklist = null,after_ranklist = null;
    if (!contest) {
      contest = await Contest.create();
      ...
      after_ranklist = await ContestRanklist.create();
      ...
      after_ranklist = contest.after_ranklist;
      if(!after_ranklist){
        after_ranklist = await ContestRanklist.create();
      }
    }
    try {
      ...
      after_ranklist.ranking_params = JSON.parse(req.body.ranking_params);
    } catch (e) {
      ranklist.ranking_params = {};
      after_ranklist.ranking_params = {};
    }

    await ranklist.save();
    await after_ranklist.save();

    contest.ranklist_id = ranklist.id;
    contest.after_ranklist_id = after_ranklist.id;
...
app.get('/contest/:id/submissions', async (req, res) => {
  try {
    ...
    let problems_id = await contest.getProblems();
    const curUser = res.locals.user;
    let tmp_problem_id = problems_id[parseInt(req.query.problem_id) - 1] || 0;
    if (contest.isEnded()) {
      if(tmp_problem_id==0){
        res.redirect(syzoj.utils.makeUrl(['submissions'], { contest: contest_id }));
      }
      else{
        res.redirect(syzoj.utils.makeUrl(['submissions'], { contest: contest_id ,problem_id:tmp_problem_id}));
      }
      return;
    }
...
app.get('/contest/submission/:id', async (req, res) => {
  try {
    ...
    res.render('submission', {
      ...
      displayConfig: displayConfig,
      contest: contest,
      ubsan: judge.use_sanitize ? "On" : "Off"
    });
  } catch (e) {
    syzoj.log(e);
    res.render('error', {
