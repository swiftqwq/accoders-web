...
app.get('/admin/enable_user',async(req,res)=>{
  try{
    if (!res.locals.user || !res.locals.user.is_admin) throw new ErrorMessage('您没有权限进行此操作。');
    let users=await User.find();
    res.render('admin_enable_user',{
        users:users
    });
  }catch(e){
    syzoj.log(e);
...
app.get('/admin/disenable_user',async(req,res)=>{
  try{
    if (!res.locals.user || !res.locals.user.is_admin) throw new ErrorMessage('您没有权限进行此操作。');
    let users=await User.find();
    res.render('admin_disenable_user',{
        users:users
    });
  }catch(e){
    syzoj.log(e);
...
app.post('/admin/enable_user',async(req,res)=>{
  let id=req.body.id;
  let user=await User.findById(id);
...
app.post('/admin/disenable_user',async(req,res)=>{
  let id=req.body.id;
  let user=await User.findById(id);
  user.is_banned=true;
  let record=await Record.create({
...
app.post('/admin/rating/export', async (req, res) => {
  try {
      let curUser = res.locals.user;
      if (!curUser || !curUser.is_admin) throw new ErrorMessage('您没有权限进行此操作。'); //only admin that who can execute this option
      let contests_id =  req.body.contests_id;
      let type = req.body.type;
      if(!contests_id || contests_id.length < 1)throw new ErrorMessage('参数不正确。');
      contests_id = contests_id.sort(c => Number(c));
      let markdown_table = `<div class="divscroll"><table class="ui celled table table_nowrap">
      <thead><tr><th>排名</th><th>姓名</th>`;
      let players = {};
      let users = [];
      let contests = [];

      for(let contest_id of contests_id) {
          let contest = await Contest.findById(contest_id);
          if(!contest || contest.type === 'test') continue; //只计算考试排行
          await contest.loadRelationships();
          //if(type == 1 && !contest.ranklist || !contest.ranklist.c_ranklist['1']) continue; //只计算有排行的考试
    if(type == 2 && !contest.ranklist || !contest.ranklist.ranklist['1']) continue; //赛后排行

          markdown_table += `<th>${contest.title}</th>`;
          contests.push(contest_id);

          for (let i = 1; i <= contest.ranklist.ranklist.player_num; i++){
              //for(let player_id in contest.ranklist.c_ranklist){
              let player = await ContestPlayer.findById(contest.ranklist.ranklist[i]); //获取比赛player

              if(!player){
                  console.log("player %s not exist...");
                  continue;
              }

              let user_id = player.user_id;
              if(!players[user_id]){
                  let user = await User.findById(user_id);  //获取用户
                  if(!user) continue;
  ...
  let cur_score = (type == 1) ? player.c_score : player.score;
              players[user_id][contest_id] = cur_score; //计算本次比赛分数
              players[user_id].total_score += Number(cur_score); //累加总分
          }
      }
      for(let player_id in players)users.push(players[player_id])

      markdown_table += '<th>总分</th></tr></thead>\n<tbody>'; //表头结束

      users = require("lodash").orderBy(users, ["total_score", "username"], ["desc", "asc"]); //排序
      let rnk=0;
      for(let user of users) {
          rnk+=1;
          markdown_table += `<tr><td>${rnk}</td><td>${user.username}</td>`;
          for(let cid of contests) markdown_table += `<td>${user[cid] ? user[cid]: 0}</td>`;
          markdown_table += `<td>${user.total_score}</td></tr>\r\n`;
      }
      markdown_table += "</tbody></table></div>";
      let moment = require("moment");
      let title = `${moment().format("YYYYMMDD")}排行榜`;
      let article = await Article.create({title: title, content: markdown_table, user_id: curUser.id, problem_id: null, public_time: syzoj.utils.getCurrentDate(), sort_time: syzoj.utils.getCurrentDate()});
      await article.save();
