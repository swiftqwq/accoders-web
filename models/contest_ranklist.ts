    ...
    if (contest.type === 'noi') {
      for (let player of players) {
        ...
        for (let i in player.score_details) {
          ...
          if (player.score_details[i].score != null) {
            ...
            player.score_details[i].weighted_score = Math.round(player.score_details[i].score * 1.0);
      ...
      for (let player of players) {
        ...
        for (let i in player.score_details) {
          ...
          if (judge_state != undefined && player.score_details[i] && player.score_details[i].score != 0) {
            player.timeSum += (judge_state.submit_time - contest.start_time);
          }
          if (player.score_details[i] && player.score_details[i].unAcCount && player.score_details[i].score != 0){
            player.timeSum += player.score_details[i].unAcCount*20*60;
          }
        }
      }
