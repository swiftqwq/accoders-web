  ...
  async updateScore(judge_state) {
    ...
    if (this.contest.type === 'ioi') {
      if (!judge_state.pending) {
        ...
        this.score_details[judge_state.problem_id].unAcCount=0;
        ...
        let fistMaxScoreSubmission = null;
        for (let x of arr){
            if(x.score == maxScoreSubmission.score){
                fistMaxScoreSubmission=x;
                break;
            }
        }
        for (let x of arr){
            if(x.time<fistMaxScoreSubmission.time&&x.score<fistMaxScoreSubmission.score&&x.status!='Compile Error'&&x.status!='No Testdata'&&x.status!='Judgement Failed'&&x.status!='System Error'){
                this.score_details[judge_state.problem_id].unAcCount+=1;
            }
            else if(fistMaxScoreSubmission.score == 0&&x.status!='Compile Error'&&x.status!='No Testdata'&&x.status!='Judgement Failed'&&x.status!='System Error'){
                this.score_details[judge_state.problem_id].unAcCount+=1;
            }
        }
