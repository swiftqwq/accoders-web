...
import RatingCalculation from "./rating_calculation";
...
export default class JudgeState extends Model {
  ...
  @TypeORM.Column({ nullable: false, type: "boolean", default: 0 })
    ...
    if (this.type === 0) {
      ...
      if (this.problem_id == -233) {
        if (this.status == Status.ACCEPTED || this.status == Status.WRONG_ANSWER || this.status == Status.PARTIALLY_CORRECT) {
          ...
          if (msg != "The selected location is illegal") {
            let message = msg.split('|');
            let val = parseInt(message[1]), mul = parseInt(message[2]);
            let user = await User.findById(this.user_id);
            user.rating += val * (mul - 1);
            const newcalc = await RatingCalculation.create({ contest_id: -233 });
            await newcalc.save();
            ...
            newHistory.rating_calculation_id = newcalc.id;
  ...
  async rejudge() {
    await syzoj.utils.lock(['JudgeState::rejudge', this.id], async () => {
      ...
      if (!syzoj.config.enable_rejudge) {
